...
class SpEditProfileScreen extends StatefulWidget {
  @override
  _SpEditProfileScreenState createState() => _SpEditProfileScreenState();
}
...
//to get the image from local gallery
  ...
  Widget _builddistrictField() {
    return DropdownButtonFormField(
      ...
      value: _currentUser.servicearea,
      onSaved: (String value) {
        _currentUser.servicearea = value;
      },
      onChanged: (newValue) {
        setState(() {
          _currentUser.servicearea = newValue;
        });
      },
      items: _district.map((servicearea) {
        return DropdownMenuItem(
          child: new Text(servicearea),
          value: servicearea,
        );
      }).toList(),
    );
  ...
  Widget _buildpersoninchargeField() {
    return TextFormField(
      ...
      initialValue: _currentUser.personincharge,
      keyboardType: TextInputType.text,
      style: TextStyle(fontSize: 14),
      onSaved: (String value) {
        _currentUser.personincharge = value;
      },
    );
  }
